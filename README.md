...
The product JSON provides the title, category, main image, detail image, SKU image, SKU sizes, stock, group price, single price, and reference price. CLI flags can still override individual values for one-off tests. Detail images are validated as source data but skipped by the draft automation because Pinduoduo can auto-fill product details from carousel images when details are not edited.
