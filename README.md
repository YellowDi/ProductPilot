...
The product JSON provides the title, category, main image, detail image, SKU image, SKU sizes, stock, group price, single price, and reference price. CLI flags can still override individual values for one-off tests. Detail images are uploaded by the draft automation when they are present in the source data.
